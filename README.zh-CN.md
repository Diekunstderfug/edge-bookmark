...
**Plan** 标签和 **LLM** 标签提供了影响方案生成和执行的设置项：
...
`config/rules.yaml` 默认设置了 `protect_root_loose_bookmarks: true`，这会保护直接挂在 `/收藏夹栏`、`/其他收藏夹` 等受保护根目录下的散落书签，不让 AI 随意移动，因为缺乏上下文的盲目搬运风险较高。如果需要整理这些书签，可以：

1. 编辑 `config/rules.yaml`，将 `protect_root_loose_bookmarks` 改为 `false`，或
2. 在 **Scope** 中指定一个子文件夹 — 该文件夹树内的所有内容不受此限制。
