...
├── tests/                  # unittest suite (6 files, stdlib only)
...
| Extension popup | `extension/popup.js` + `popup.html` | UI, form persistence, AES-GCM key storage |
| Extension background | `extension/service_worker.js` | chrome.bookmarks API operations, plan execution |
| Extension AI planner | `extension/ai_planner.js` | HTTPS fetch against OpenAI-compatible APIs (SDK-free) |
...
| `executeReviewedPlan` | Function | `extension/service_worker.js:59` | Extension plan execution |
...
- **No pytest**: Tests use stdlib `unittest.TestCase` + `unittest.main()`, run via `python3 -m unittest discover -s tests`
